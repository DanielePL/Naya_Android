...
-- =====================================================
-- 3. USER COACH RELATIONSHIP TABLE
-- =====================================================
-- Track which users have coaches (for storage decisions)
...
CREATE INDEX idx_user_coach_relationships_user_id ON user_coach_relationships(user_id);
CREATE INDEX idx_user_coach_relationships_coach_id ON user_coach_relationships(coach_id);
CREATE INDEX idx_user_coach_relationships_status ON user_coach_relationships(status);
...
-- =====================================================
-- 4. ADD VIDEO_STORAGE_TYPE TO USERS TABLE
-- =====================================================
-- Track if user uses device or cloud storage

ALTER TABLE auth.users
ADD COLUMN IF NOT EXISTS video_storage_type TEXT DEFAULT 'device'
CHECK (video_storage_type IN ('device', 'cloud'));

-- Function to automatically set cloud storage when coach is added
CREATE OR REPLACE FUNCTION set_cloud_storage_on_coach_assignment()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.status = 'active' THEN
        -- Update user to use cloud storage
        UPDATE auth.users
        SET video_storage_type = 'cloud'
...
CREATE TRIGGER trigger_set_cloud_storage
AFTER INSERT OR UPDATE ON user_coach_relationships
FOR EACH ROW
WHEN (NEW.status = 'active')
EXECUTE FUNCTION set_cloud_storage_on_coach_assignment();

-- =====================================================
-- 5. UPDATE WORKOUT_SETS TABLE
-- =====================================================
-- Add video URL and storage type fields

ALTER TABLE workout_sets
ADD COLUMN IF NOT EXISTS video_url TEXT,
ADD COLUMN IF NOT EXISTS video_storage_type TEXT DEFAULT 'device' CHECK (video_storage_type IN ('device', 'cloud')),
ADD COLUMN IF NOT EXISTS video_thumbnail_url TEXT,
ADD COLUMN IF NOT EXISTS video_uploaded_at TIMESTAMPTZ;

-- Index for video queries
CREATE INDEX idx_workout_sets_video_url ON workout_sets(video_url) WHERE video_url IS NOT NULL;
...
5. Example: Add coach to user
   ...
   -- This automatically sets user's video_storage_type to 'cloud'
*/
